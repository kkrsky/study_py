...
class SortableTableDemo(ttk.Frame):
    ...
    def _open_tag_selector_dialog(self, item_id: str, col_name: str) -> None:
        ...
        available_tags = list(self.table_config.available_tags)
        ...
        selected = set(self._parse_tag_text(current_value))
        ...
        ttk.Label(body, text="UseをONにしてOKを押すと、選択したタグ名をセルへ反映します。").pack(
            anchor="w", pady=(0, 6)
        )
        ...
        visible_rows = min(max(len(available_tags), 6), 12)
        tag_tree = ttk.Treeview(
            table_wrap,
            columns=("use", "tag_name", "description"),
            show="headings",
            selectmode="browse",
            height=visible_rows,
        ...
        tag_tree.column("tag_name", width=180, minwidth=130, anchor="w", stretch=False)
        tag_tree.column("description", width=360, minwidth=220, anchor="w", stretch=True)
        ...
        row_iid_by_tag: Dict[str, str] = {}
        tag_use_state: Dict[str, bool] = {tag: (tag in selected) for tag in available_tags}
        for tag in available_tags:
            use_text = CHECK_ON if tag_use_state[tag] else CHECK_OFF
            description = self.table_config.tag_descriptions.get(tag, "")
            row_iid = tag_tree.insert("", "end", values=(use_text, tag, description))
            row_iid_by_tag[tag] = row_iid

        def refresh_row_use(tag: str) -> None:
            row_iid = row_iid_by_tag.get(tag, "")
            if not row_iid:
                return
            row_values = list(tag_tree.item(row_iid, "values"))
            if len(row_values) < 3:
                return
            row_values[0] = CHECK_ON if tag_use_state.get(tag, False) else CHECK_OFF
        ...
        def toggle_use_for_row(row_iid: str) -> None:
            ...
            if len(row_values) < 2:
                return
            tag_name = str(row_values[1]).strip()
            if not tag_name:
                return
            tag_use_state[tag_name] = not tag_use_state.get(tag_name, False)
            refresh_row_use(tag_name)
        ...
        def on_ok() -> None:
            selected_tags = [tag for tag in available_tags if tag_use_state.get(tag, False)]
            vals[col_index] = self.table_config.tag_separator.join(selected_tags)
            self.tree.item(item_id, values=vals)
            self._rebuild_filter_values()
            self._apply_filters()
            dialog.destroy()
    ...
    def _rebuild_filter_values(self) -> None:
        ...
        for col in self.columns:
            ...
            if col in self.tag_selector_cols:
                values.extend(self.table_config.available_tags)
