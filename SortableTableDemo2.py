...
class SortableTableDemo(ttk.Frame):
    ...
    def _insert_demo_rows(self):
        rows = [
            (1, "Buy apples", "Food", "New", "Mid", True, 10, 120.5, 0, "2026-02-05", "Run", "From supermarket"),
            (2, "Make coffee", "Drink", "Hold", "Low", True, 1, 980.0, 20, "2026-02-01", "Run", "Try new beans"),
            (3, "Daily jog", "Daily", "Done", "High", False, 1, 0.0, 100, "2026-01-25", "Run", "5km"),
            (4, "Write report", "Work", "New", "Urgent", True, 1, 0.0, 10, "2026-02-10", "Run", "Draft first"),
            (5, "Blender practice", "Hobby", "Hold", "Mid", True, 1, 0.0, 35, "2026-02-12", "Run", "Modeling"),
        ]
        for r in rows:
    ...
    def add_row(self):
        ...
        row = (
            ...
            self.dropdown_values["priority"][0],
            True,
            0, 0.0, 0, today, "Run", ""
        )
    ...
    def _begin_combobox_edit(self, item_id: str, col_name: str):
        ...
        cb = ttk.Combobox(self.tree, values=self.dropdown_values[col_name], state="readonly")
        cb.set(current if current in self.dropdown_values[col_name] else self.dropdown_values[col_name][0])
